...
import com.coherentsolutions.pot.insuranceservice.controller.AdminUserManagementController;
...
class AdminUserManagementControllerTest {
  ...
  void setUp() {
    ...
    testUserDto = UserDto.builder().id(testUserId).firstName("John").lastName("Doe")
        .email("john.doe@example.com").status(UserStatus.ACTIVE)
        .functions(Set.of(UserFunction.COMPANY_CLAIM_MANAGER)).dateOfBirth(LocalDate.of(1990, 1, 1))
        .build();
  }
  ...
  void shouldGetUserDetailsById() throws Exception {
    // Given
    when(userManagementService.getUsersDetails(testUserId)).thenReturn(testUserDto);

    // When & Then
    mockMvc.perform(get("/v1/users/{id}", testUserId).contentType(MediaType.APPLICATION_JSON))
        ...
        .andExpect(jsonPath("$.functions").isNotEmpty()).andExpect(status().isOk())
