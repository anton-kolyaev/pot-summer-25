 ...
 * JpaRepository} for basic CRUD operations and {@link JpaSpecificationExecutor} for filtering
 * capabilities.
 */
public interface UserRepository extends JpaRepository<User, UUID>, JpaSpecificationExecutor<User> {
  ...
  /**
   * Retrieves a {@link User} by its ID or throws {@link ResponseStatusException} with 404 NOT FOUND
  ...
  void updateUserStatusByCompanyId(
      @Param("companyId") UUID companyId, @Param("status") UserStatus status);

  /** Updates the status of users with the given list of user IDs. */
  @Modifying
  @Query("UPDATE User u SET u.status = :status WHERE u.id IN :userIds")
  void updateUserStatusByIds(
      @Param("userIds") List<UUID> userIds, @Param("status") UserStatus status);
}
