...
/**
 * Provides JPA Specifications to filter {@link User} entities based on various
 * criteria encapsulated in {@link UserFilter}.
 */
public class UserSpecification {

    /**
     * Creates a Specification for filtering {@link User} entities according to
     * the criteria defined in the given {@link UserFilter}.
     */
    ...
    private static Predicate companyIdPredicate(UserFilter filter, Root<User> root,
    ...
    private static Predicate namePredicate(UserFilter filter, Root<User> root,
            CriteriaBuilder criteriaBuilder) {
        if (hasText(filter.getName())) {
            ...
            Predicate firstNameLike = criteriaBuilder.like(criteriaBuilder.lower(root.get("firstName")),
                    pattern);
            Predicate lastNameLike = criteriaBuilder.like(criteriaBuilder.lower(root.get("lastName")),
                    pattern);
    ...
    private static Predicate emailPredicate(UserFilter filter, Root<User> root,
            CriteriaBuilder criteriaBuilder) {
    ...
    private static Predicate dateOfBirthPredicate(UserFilter filter, Root<User> root,
            CriteriaBuilder criteriaBuilder) {
    ...
    private static Predicate statusPredicate(UserFilter filter, Root<User> root,
            CriteriaBuilder criteriaBuilder) {
    ...
    private static Predicate ssnPredicate(UserFilter filter, Root<User> root,
            CriteriaBuilder criteriaBuilder) {
    ...
    private static Predicate functionPredicate(UserFilter filter, Root<User> root,
            CriteriaBuilder criteriaBuilder) {
